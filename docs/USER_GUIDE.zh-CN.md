...
1. [项目能力边界](#1-项目能力边界)
2. [安装与首次配置](#2-安装与首次配置)
3. [模板浏览、搜索与提交绘图任务](#3-模板浏览搜索与提交绘图任务)
...
SFL 的核心纪律是:**选择一个或多个模板并提交绘图任务,再按需要检查精确预览和确认后物化**。
物化(materialize)是把确认的模板文件复制到项目,不是运行代码。你可以在尚未逐张查看精确预览时先提交任务;宿主 Agent 应在创建物化计划前完成必要的精确预览和回执流程。
...
| 4. 提交绘图任务 | 你 → 宿主 App | 一个或多个已选模板进入统一 `taskItems[]` 交接,宿主必须处理全部选中项 |
| 5. 查看精确预览 | 宿主模型/App → 你 | 按需要查看选中版本的真实预览;没有 App 的宿主使用 headless 预览流程 |
| 6. 明确确认 | 你或宿主流程 | 只有精确预览后的确认才会签发本次会话的一次性预览回执 |
| 7. 核对物化计划 | 宿主模型 → 你 | 展示模板、文件集(template/full)、网络与 Source Pack/镜像策略和绝对目标目录,等你确认 |
| 8. 应用物化 | 宿主模型调用 SFL | 保持原回执和模板身份不变;目标为 `<目标目录>/<templateId>`,不覆盖已有目录 |
| 9. 检查文件并适配 | 你 + 宿主模型 | 查看文件清单与 `template.lock.json`,然后进入数据适配,尚未运行代码 |
...
选中模板后按[第 3 节](#3-模板浏览搜索与提交绘图任务)继续,数据适配见[第 5 节](#5-准备你自己的图与数据)。
...
运行时全局 Library 的 Local Published 内容仍在 `store/`。完整的 Open
Figure Modules Source Pack 是并列目录 `source-packs/open-modules/`，其中
...
绑定与物化都要展示计划再等用户确认。精确预览的 `providerId`、`exactSelector`、`previewReceipt` 必须保持不变;缺回执会返回 `preview_required`。完整 Open Figure Modules 默认使用全局 Library 下的 `source-packs/open-modules/`;固定 archive 校验成功后会自动持久化并保留 ZIP 与解压缓存。Gitee 只作为镜像传输来源,不会改变 canonical 身份。
